...
let connection = mysql.createConnection({
  ...
  user: 'root',
  password: '',
  database: 'food_order' // Default database, can be changed dynamically
});
...
// Route to get list of databases
...
app.post('/generate-query', async (req, res) => {
  ...
  if (!query || query.trim().length === 0 || !database) {
    return res.status(400).json({ error: 'Please provide a valid request and select a database.' });
  }

  // Change database in the connection
  connection.changeUser({ database: database }, async (err) => {
    if (err) {
      console.error('Error changing database:', err);
      return res.status(500).json({ error: 'Failed to connect to the selected database.' });
    }
